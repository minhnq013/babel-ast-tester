...
let Element = class extends React.Component {
    ...
    _getProperties(parser, value) {
        ...
        return properties
            .filter(
                ({ value }) => !hideFunctions || typeof value !== "function"
            )
            .filter(({ value }) => !hideEmptyKeys || value != null)
            .filter(
                ({ key }) => !hideLocationData || !parser.locationProps.has(key)
            )
    ...
    render() {
        const { focusPath, parser, level } = this.props;
        ...
        if (value && typeof value === "object") {
            ...
            if (typeof value.length === "number") {
                ...
                if (open) {
                    prefix = "{";
                    suffix = "}";
                    let elements = this._getProperties(parser, value).map(
                        ({ key, value, computed }) =>
                            this._createSubElement(key, value, key, computed)
                    );
                    content = <ul className="value-body">{elements}</ul>;
                    showToggler = elements.length > 0;
                } else {
                    let keys = this._getProperties(parser, value).map(
                        ({ key }) => key
                    );
                    valueOutput = (
                        <span>
